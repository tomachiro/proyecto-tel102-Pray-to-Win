...
configuracion() : rond_muerte_sub(10), modo_muerte_sub(0), rond_mod(5), golpe_critico(1.5f), acierto(80) {}
...
int ObtenerModoMuerteSub() {return modo_muerte_sub;}
...
void ModificarMuerteSub(int v) {modo_muerte_sub = v;}
...
public:
    jugador() : tipo_dado(6), vida(100), atq_b(10), turno(0) {
        std::strcpy(vida_c, "[----------]"); // la barra vacia por defecto
    }
    jugador(int t) : tipo_dado(6), vida(100), atq_b(10), turno(t) {
        std::strcpy(vida_c, "[----------]"); //el constructor con el turno especifico
    }
    ...
    const char* ObtenerVidaC() { return vida_c; }
    ...
    void ModificarVidaC(const char* v) { std::strcpy(vida_c, v); }
};
